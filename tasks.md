...
> **快照声明（v2.4.4 补）**：本文件是 2026-09-17 那次审计（spec.md，基线 v2.0.2）的落地清单，
> 其中的耗时、步数、覆盖率数字均为**当时实测值，会随代码演进漂移**，以 `node tools/check-all.js`
> 实际输出为准。（它也因此被 check-docs.js 排除在防漂移规则之外。）
