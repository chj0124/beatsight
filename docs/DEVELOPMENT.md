...
│   └── scan-util.js            # 三者共用的扫描工具（剥注释 / 括号配对）
└── docs/
...
| `beatsight.m2` | **旧键，只读的迁移来源** | 仅首次升级时读取；旧键存在而新键缺失 → 一次性拆分写入新键 + 备份 `beatsight.m2.bak` |
...
**写完检查器/断言后要反向验证**（v1.3.0 起的硬规矩）：把修复临时"退回"，确认目标断言**真的会失败**。没做过反向验证的测试等于没有测试——尤其对"窗口自适应""增量重绘""零布局读取"这类**没有肉眼可辨症状**的性能承诺。
...
- **CI 仍无覆盖率、无 lint 生态**（审计 P1-7 部分完成）：v1.3 补了架构约束 / 零依赖 lint / DOM 引用检查 + 失败产物上传，但**覆盖率**仍未采集。若要更全的静态规则，正路是加 `package.json` + ESLint devDependency（只进 CI、不进产物——"零依赖"约束针对的是 `file://` 直开的运行时产物，不是开发工具）
- `Viz.paintBall` 的物理动画（76 行）仍无逐帧数值断言，T23/T27 只覆盖「不抛异常 + 类名/结构不变量」
- 后台持续发声**尚缺真人验收**：自适应窗口的行为只能用假时钟断言，真实浏览器下仍需「切后台 30s、回前台核对无空隙」的耳朵验收
- `Store` 的 `beatsight.m2` 旧键迁移没有"清理点"——老用户的旧键会一直留着（只占几 KB，但概念上是个已废弃的键）。若要收敛，可在迁移成功后写一个 `beatsight.m2.migrated` 标记，或在若干版本后提示用户清理
