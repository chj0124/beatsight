...
1. **v1.4.1 修 bug：木鱼/鼓组响度不足**（2026-09-15 用户实拍反馈）：滤波噪声路径缺 makeup gain——木鱼带通 Q=8 衰减约 23dB、军鼓约 14dB、踩镲约 7dB。修法：CONFIG.timbres 加 makeup + 独立钳制上限，noiseHit 内 min(cap, peak×makeup)；新增 T44 并反向验证；最后真人试听校准
...
3. **7 天爬升训练计划**：形态视统计数据使用情况再定（「继续上次」已提供最小闭环）
