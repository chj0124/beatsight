...
### 3.15 网格 = 歌曲小节上的滚动窗口（v2.5.2 第 Ⅱ 期，改渲染层前必读）

**一句话**：曲式模式下网格的 4 行 = **这首歌正在弹的第 k…k+3 小节**，跟着播放滚；
行可以来自不同的段、不同的型。预设模式下仍是"画当前这个型的全部小节"（原行为）。
...
| `winAnchor(k)` | **手感档位就这一行**：`k` = 跟播放头滚动（默认）；`floor(k/4)*4` = 每 4 小节翻页 |
