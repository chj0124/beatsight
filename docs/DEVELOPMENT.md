...
`Controls` 是 UI 层中枢，当前**直接引用 7 个下游模块**（Presets / Editor / Settings / Ear / Arrange /
...
- **校准发声不吃补偿**（`calibPlay` 里的 `calibRaw`）：测的就是"没补偿时多晚到"。
...
`latRead / latWrite / latSuggest / latencyMs() / latState()`；向导逻辑走元素事件驱动。

**跟拍校准的数学**：8 声固定间隔（500ms）点击 → 敲击与最近一声求差（±400ms 内才配对）
→ 中位数 → 对齐 5ms 步长并钳到 [0,500]。中位数里含**人的反应时间**（约几十毫秒），
**刻意不扣**（宁可稍早、不可再晚；扣一个拍脑袋的常数只是换一种系统性偏差）；向导文案
明说"先用它、再用滑杆微调到声画重合"。样本 < 4 下 → 判为测不准。

**回归**：`tests/cases/t95-latency-compensation.js`（7 场景）；反向验证三轮定向变异
（抽掉补偿 4 红 / 退回前瞻量 1 红 / 放宽钳制 5 红）。★ **真机验收项**：向导手感与补偿后的
视听觉对齐，冒烟脚本看不出来。
...
- 覆盖率未覆盖的行**分两类、性质不同**（**条数、百分比、行号一律不抄进文档**，v2.0.5 起一律以 `node tools/check-coverage.js` 的输出为准——它会打印总量、分区占比与未覆盖行的绝对行号）：
  ...
  **另一类是不可达分支**——**Ear** 的 `durName` 里 192/96 与 6 这几档时值（内置库与听辨出题组都不用这几档时值，`durName` 也不对外导出）。原先还列了 144/18 两档，实为 `VALID_T` 之外的死分支（任何经校验的型都到不了），已于 P3 工程卫生清理中删除。
  两类都不为了数字去造人工状态点亮它。
  ★ **v2.18.0 复核（装配区那一档，别再重新发现一遍）**：17 个分区里**装配区**是唯一明显低于
     邻居的一条线（其它 16 条都在 98–100%，它在 96% 上下）。逐行核过一遍，那 38 行**全部**属于
