...
| T85 | 侧栏分区折叠（v2.10.0；v2.13.0 改出厂默认，4 个子场景）：出厂默认**节拍+自定义展开、扫弦收起**；★★ 加载判据是**存在性 + 严格 true**——「没存过」用出厂默认、「存过显式 false」尊重用户（老用户存档里就是显式 false，只判 `=== true` 会让改默认值对他们等于没改）/ 标题即按钮（`role=button` + `tabIndex=0` + Enter/Space，**不是** `role=switch`）/ 只置 `hidden` 不删节点（children 数与对象身份不变）/ 开合记忆在热键且重载恢复、脏值白名单回落收起 / 列表重建后由收尾 `applyFold()` 重新套用 |
...
| T89 | 范围短于窗口时的「下一行」落点（v2.10.8，5 个场景 / 44 条断言）：待命球与 `.next` 预告格**按节目单算**而不是位置式 +1 —— **W > L 全组合**的落点表（含"每行都观察到过"的防空断言守卫）/ 两者**逐位一致**（同一份口径的两个消费者不许分叉）/ `W ≤ L` 与整首连播的**对照**（本修复不许动它们）/ **用户实报路径端到端**（拖侧栏滑块选 1 小节 + 4 行档）/ `loop=OFF` 不许出现假回卷、范围播完球即隐藏 / **跨页回卷仍走位置式的已知边界**（钉住防漂移） |
| T90 | 控件布局搬家（v2.10.11，5 个场景 / 56 条断言）：拍号整块搬进时值可视化卡、落在「同屏行数」**之后**且 `#accGroup` 跟随（含"搬块不换内容"：6 档拍号无一丢失）/ 音色与音量搬进当前节奏型行、**都在 `#editBtn` 之前**且保持 音色 → 音量 顺序（label 已缩短成「音色」「音量」，故 title / aria-label 必须一并搬来）/ 走带卡收口（只剩 BPM + Swing、`class="group"` 恰好 2 个、`.vdivider` 恰好 1 条——多留一条会连成双竖线）/ 两条 CSS 契约（`.viz-toggles .toggle-pill` 左内边距归零且**没**动全局 `.toggle-pill`；`.pattern-head` 桌面 24px、窄屏 16px）/ **搬家不得改变行为契约**（id / 初始态 / `accGroup.hidden` 初值 / 点档位仍生效的接线）。★ 用标记字符串断言（桩不解析 HTML），真几何在 `tools/smoke.js` 的 `layoutProbe()`（桌面 + 390px 各一遍） |
| T90（v2.10.12 追加） | **再搬家**：音色进设置弹窗、编辑节奏型去侧栏「编排曲式」左边、当前节奏型行右侧只剩音量（`.ph-controls` 撤掉）；设置弹窗 9 项分三组（外观与辅助 / 发声 / 数据与说明）；练习量与练习统计删除（`t48` / `t50` 整文件删除，`t37` 改名） |
| T91 | **老数据搬家回归矩阵**（v2.11.x，6 个场景）：旧热键 `beatsight.m2`（v1 冷热未拆）拆进冷热两键后 `trainer.start/last/plan` **一个都不留**（只剩 `on/target/step/everyN`）/ 写盘失败时**保留旧键**以便下次重试、半迁移状态不动旧键 / **老全量数据包**（带已删的 `log` 一类、歌词段叫 `lines`、块引用 `{type:"builtin",idx:0}`）静默忽略 `log` 且其余四类照进、★ 弹窗不再出现「练习记录 undefined 场」 / `arrangeSel` 段下标→小节区间的**两阶段**迁移（取走即清空、`v:2` 不再二次换算）/ **脏值矩阵**（一批字段同时脏：照常加载、循环区间仍保证 `from ≤ to`、壁纸键坏了不影响热键）/ **往返一致**（读进来→再存→再读，字段不会越迁越多）。★ 造包必须用**老形状**，拿当前形状造包测不出迁移 |
| T92 | **背景壁纸**（v2.12.0；v2.13.0 扩为**三态**，9 个场景）：格式只认 JPG/PNG/WebP 且**按魔数判**（含"MIME 撒谎但内容是文本"、SVG 与 GIF 被拒；MIME 与内容不符时**以内容为准**）/ 进 CSS 前的值不含引号（★ 真 PNG 头 + `")` 负载——只有正则那一层挡得住）/ 体积上限与带数字的理由 / ★★ **读档三态**（键不存在=出厂默认图且**不落盘**、`"default"` 记号=默认图+自定义遮罩、`null`=用户明确移除且默认图**不复活**），脏值一律回**出厂默认图**（v2.12.0 是回"没有壁纸"）/ ★★ 遮罩只认**数值类型**（`Number(null)===0` 会把壁纸读成"完全不压暗"）/ **不进热键、不进导出包**、遮罩与图片同键同写 / ★★ 移除写 `{img:null}` 哨兵而**不是删键**（删键 = 刷新后默认图又冒出来）、「恢复默认」= 删键（不把 381 KB 常量抄进存储）/ 遮罩两级口径（`input` 不落盘、`change` 才写，默认图上落的是**记号**）/ 选图的四种失败都有话说且**画面停在默认图**（失败无副作用）、★ 同一文件连选两次仍能触发 / 换主题换遮罩色。★ `atob` 对**非法长度**也抛 `InvalidCharacterError`，而 `wallCheck` 在**加载期**被调用——不接住就是整页白屏，已单列断言 |
| T93 | **首次打开的默认状态**（v2.13.0，5 个场景）——用户提的四件事的**验收面**：★★ 默认选中示例曲《在他乡》但**绝不自动播放**（三条独立证据：`S.playing===false`、调度器 interval 数为 0、调度游标停在 0 小节；并配一条**正控**——同一个桩里真的播一次，三条立刻反过来，以免"音频栈没起来"造成假绿）/ 选中状态**立刻落盘**并在下次打开被恢复、带出闩同时落上 / ★★ 闩已落的**老用户**不自动选中（用真实冷键数据造"老用户"，别用光秃秃的 `loadApp()`，那是"闩落上但库里没有"的异常组合）/ 同屏行数 4 与拍号 4/4（**状态与控件高亮两边都判**）/ 首开四件套一屏之内同时成立（折叠态 + 行数拍号 + 默认壁纸不落盘 + 示例曲已选中且未播）。★ 这一组的前提被改写过的还有 T85 / T91e / T68g / T88d / T88j（详见那些文件里的"v2.13.0"注释） |
| T94 | **空格键全局 = 播放/暂停**（v2.13.1，7 个场景）——用户实报「点过其他功能后空格就哑了」的验收面：★★ 滑杆（`input[type=range]`）焦点下仍切播放（旧规则把它排除在外，本次修复的主角）/ 真按钮焦点下切播放，且 `preventDefault` 拦掉原生空格激活（不拦会双动作）/ 侧栏预设项**不再消费**空格（向**另一项**发空格选中不变）+ 焦点在项上仍切播放 / ★ 「正在输入文字」是唯一例外（text / number / textarea / contenteditable 四种形状 + range 反例对照）/ 修饰键（Ctrl/Cmd/Alt+Space）与长按 `repeat` 不抢 / 面板打开时空格不误触播放（回归钉子）。★ 反向验证两轮定向变异：退回全局规则 → 10 条转红；退回 `itemKeys` → 3 条转红（含一条**第一版写弱、当场加严**的断言） |
| T95 | **音频延迟补偿**（v2.14.0，7 个场景）——蓝牙耳机延迟的验收面：★★ 读档校验矩阵（无键不落盘 / 坏 JSON / 逐条淘汰 / ms 钳制与步长对齐 / 名字净化 / 条数上限 / currentId 回落）/ ★★ 发声统一提前（前 N 声逐位前移 200ms，且 **onset 表与 loopStart 一字不动**——对照组断言）/ ★★ 前瞻量 = 窗口 + 补偿（单轮就排到"边缘那颗"，退回旧量即红）/ 建议值计算（中位数 / 剔非数 / 钳制 / 步长 / 样本下限）/ 校准发声**不吃补偿**（校准结束后补偿恢复）/ 向导端到端（跟拍 → 建议 180ms → 采用 → 落盘；抢拍忽略、样本不足、取消三态）/ 配置 CRUD（新建起点 = 当前值不突变 / 空名不改 / 删除落到剩下那条 / 两级落盘 / 重载恢复）。★ 反向验证三轮变异：4 / 1 / 5 条转红 |
| T96 | **窄屏自适应分片**（v2.15.0，5 个场景）——「手机上竖屏一行一小节挤得看不清」的验收面：★ 判据边界矩阵（`chooseRowBeats` 纯函数：600/300/140px × 十六分、稀疏型不拆、奇数拍退到每行 1 拍、脏输入兜底）/ ★★ 同一十六分型 **600px 与 300px 的对照**（16 格 vs 8 格、座次尺只报本行拍、拍号从第 3 拍数起、网格线 3 条 → 1 条）/ ★★ 跨行长音 = 续接片段（`.cutl/.cutr` 两侧各一段、宽度按截断后算、**截断片段不打时值标签**、宽屏对照组有标签）/ 帧路径翻行（播过第 2 拍换行、状态栏仍报「第 1 小节 · 3」的小节口径）/ 曲式模式 + 歌词轨的片口径（歌词行跨度 = 96t 而非 192t、跨片字各归各行）。★ 两处重写 className 的路径（`setCell` / `resetForStop`）各有一条"续接标记仍在"的钉子。★ 反向验证三轮变异：35 / 2 / 1 条转红（第三轮复现了实施期真实漏改的 `audioPosAt` 片内折算）|
> **编号说明**：`section()` 的 T 编号目前有重复——T38 与 T41 各被用两次
> （`t37-training-and-keepalive.js` 与 `t30-wiring-and-lifetime.js`）。这只是日志标签重复，
> 不影响执行与断言；但**新增用例请从当前最大值往上取号，别复用**。
> ★ v2.10.12：**`t48-practice-limit.js` 与 `t50-practice-coverage.js` 整文件删除**
> （练习量与练习统计两项功能随用户要求删除），`t37-stats-and-training.js` 改名为
> `t37-training-and-keepalive.js` 并删掉其中 Stats / 练习记录 / 统计增强 / 导出记录四组场景。
> ★ v2.10.4 订正：此前的注记写着「v2.0.1 起为 T55，下一个是 T56」，而实际最大号早已到 **T87**
> （T56–T84 也早已有用例落地）——**v2.10.11 起 T89 / T90 也已占用，v2.13.0 起 T93 也已占用，v2.13.1 起 T94 也已占用，v2.14.0 起 T95 也已占用，v2.15.0 起 T96 也已占用，下一个空号是 T97**。
