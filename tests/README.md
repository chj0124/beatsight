...
| T51 | 曲式数据模型（v2.0.0 S2，6 个子场景）：**结构校验**（18 种脏结构全部被拒、上限边界、总小节 256/272、字段白名单）/ id 生成与 upsert 更新语义 / 冷键加载（坏条目丢弃、坏 JSON、往返一致、写失败可见）/ **`arrangeProblems` 的引用存在性与拍号一致**（这条在共享区，因为要 resolveRef）/ `playMode` 与 `arrangeSel` 的脏值回退与越界钳制（含 `to < from` 必须抬到 from）|
...
| T88 | 侧栏「播放范围」双滑块（v2.10.4，11 个场景 / 71 条断言）：控件形状与 **1-based 取值域** / 拖出任意区间（★ 断言 `from !== to`——旧段号胶囊永远拿不到）/ **双向钳制**（起点越过终点会把终点顶走，反之亦然；含非数字脏值回退不进 NaN）/ **`input` 与 `change` 两级契约**（只发 input 不得切模式、不写主界面反馈）/ 重合 = 单段（与旧胶囊逐位等价，含停播换型）/ 读数与 `aria-valuetext` 的段名翻译 / 填充条几何（`style.left`·`width` 百分比）+ **重合时 thumb 的 z-index 翻转** / **播放期间滑块纹丝不动**（范围≠位置）/ 越界范围按段数收窄 / 与「整首连播」高亮同源 / 曲式被删后的悬空引用守卫 |
