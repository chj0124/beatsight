...
/* ================= 场景 T69b：网格行数 = 同屏行数档位（型长决定"窗口里装什么"） ================= */
section("T69b 型长自由化 · 网格行数 = 同屏行数档位，型长决定窗口内容（v2.10.2 窗口化）");
{
  /* ★ 契约变更（v2.10.2）：此前"行数 = 型的小节数"，型长自由化直接反映为行数。
     现在**两种模式都是「N 行窗口」**（N = 同屏行数档位，默认 4），行数由档位唯一决定：
       型长 < N → 绕回重复铺满 N 行；型长 > N → 按 N 小节翻页。
     型长依然是真实存在的约束，只是体现在**窗口里装的是哪几个小节**上
     （内容层面的断言见 t87-preset-row-window）。 */
  const one = withPattern(1);
  eq(rowsOf(one.els).length, 4, "★ 1 小节的型 + 默认 4 行档 → 4 行（同一小节绕回铺满）");
