/* BeatSight 自动化测试 · 曲式模式的主界面收口（v2.6.3）
   ...
   ② 曲式播放中切轨，画面与节目单分裂：连播中切到普通轨，S.sel 被 ensureValidForTrack
     回退成四分基础，refreshAfterPatternChange 拿 curPattern()（= S.sel）去 resyncToNow +
     重建画面 → 标题/网格变成四分基础，节目单却还在走；侧栏高亮与播放内容完全分裂。
     修法：曲式播放中的取型唯一来源是节目单——arrangePlayPattern()（游标处的块），
     refreshAfterPatternChange 的接续目标与 applyPatternChange 的回退链都改走它。 */
"use strict";
...
/* 与 t68 同一口径：示例曲只在扫弦轨渲染；seedDemo:false 让它真的带出（7 型 + 曲式） */
...
/* ================= 场景 T73b：曲式播放中切轨 → 画面跟着节目单，不跟 S.sel（图二） ================= */
section("T73b 曲式播放中切轨 · ★ 标题/网格仍跟节目单（S.sel 回退不拽走画面）");
{
  const { beat, els } = loadDemo();
  ...
  ok(homeName.includes("在他乡"), "前提：连播中标题是节目单的型（实际「" + homeName + "」）");

  /* 切到普通轨：S.sel 会被 ensureValidForTrack 回退成第一个普通型（四分基础）——
     那是"退回预设模式后选谁"的数据，不是"现在在播谁"。画面必须继续跟节目单 */
  els["trackRow"].children[0].fire("click");
  eq(beat.Store.S.playMode, "arrange", "切轨不动播放模式（轨只是预设库的筛选器）");
  const afterName = els["patternName"].textContent;
  ok(afterName.includes("在他乡"),
    `★ 切轨后标题仍是节目单的型（实际「${afterName}」）——旧实现这里变成「四分基础」`);
  ...
  eq(afterName, (beat.arrangePlayPattern() || {}).name || "",
    "★ 标题与节目单游标处的型严格同名（取型唯一来源 = 节目单）");

  /* 继续走：节目单照常推进，标题每过一个块都换——证明时间轴没被那次切轨拽坏 */
  let sawOther = false;
  for (let k = 0; k < 40 && !sawOther; k++){
    drive(ac, beat, 0.5);
    const nm = els["patternName"].textContent;
    if (nm.includes("在他乡") && nm !== afterName) sawOther = true;
    eq(nm, (beat.arrangePlayPattern() || {}).name || "",
      `推进中标题始终 = 节目单游标处的型（第 ${k + 1} 步，实际「${nm}」）`);
  }
  ok(sawOther, "★ 节目单继续推进到了别的段/型（没有被切轨卡死或拽回）");
  beat.Controls.stop();
}
