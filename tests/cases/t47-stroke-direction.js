...
section("T47b 扫弦方向 · 窄格自动隐藏（不裁半支箭头）/ 六线底纹 / zone→弦区跨距");
{
  ...
  /* ★ v2.4.2 的临界点变了（STRUM_MIN_W 20 → 14），而**合法时值的最小档就是 6t**
     ...
     ★★ v2.15.0（行内切分）之后再调一档：**窄行宽不再等于窄格子**——自适应会先把一小节
        拆成几行（每行 ≥1 拍、且保证最短格尽量 ≥ SLICE_MIN_CELL=24px），所以"192px 行宽 ×
        6t"算出来的是"每行 1 拍、格子 24px"（反而够宽）。要让 6t 格重新落到 14px 以下，
        得把行宽压到连"每行 1 拍"都救不回来：**96px 行宽 → 6t 格 = 12px < 14** ✓，
        此时切分档 = 每行 1 拍（K=4），一行的格子分布是：
  ...
  /* v2.15.0：断言按**切片后的行**重新对位（96px 行宽 → 每行 1 拍，见上方布局表） */
  ok(!!strumOf(els, 0, 0), "48t 宽格生成了箭头");
  eq(disp(0, 0), "", "48t 宽格（96px 行宽下 = 92px）：箭头可见（display 未置 none）");
  ok(!!strumOf(els, 1, 0), "6t 窄格也生成了箭头（只是被隐藏，不是没生成）");
  eq(disp(1, 0), "none", "6t 窄格（96px 行宽下 = 12px < 14）：箭头隐藏");
  eq(disp(3, 2), "none", "末位 6t 窄格（第 4 行第 3 格）：同样隐藏");
  ok(strumOf(els, 1, 1) === null, "无 dir 的格不生成箭头（不是隐藏，是压根没有——它是 96t 的截尾片段）");
