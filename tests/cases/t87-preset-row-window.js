...
section("T87c 预设窗口 · 4 小节的型 + 2 行档 ⇒ 每 2 小节翻一页（Q2）");
{
  ...
  ok(flips.some(f => f.to === "4,8" && f.from === "1,2"),
     "翻页方向正确：第 1-2 小节 → 第 3-4 小节（实际 " + JSON.stringify(flips) + "）");
  ok(flips.every(f => f.row === 0),
     "★★ 每一次翻页那一帧的当前行都是**新页第 0 行**（端点的小节号已按新页锚换算）——"
...
section("T87d 预设窗口 · 6 小节的型 + 4 行档 ⇒ 走到尾部绕回开头（同 arrWindowPat 口径）");
{
  ...
  eq(flips.length, 1, "只翻一页（实际 " + JSON.stringify(flips) + "）");
  eq(flips[0].row, 0, "★★ 翻到页锚 4 那一帧的当前行 = 新页第 0 行（第 5 小节落在第 0 行）");
}
