/* BeatSight 自动化测试 · 跳段按钮基准 + 翻页预告行（v2.7.0）
   ...
   ② 整首连播时球永远在第 1 行跳（滚动窗口：当前小节恒为第 1 行）。
     ...
     用户方案：页内第 4 小节期间把第 1 行（最陈旧的历史）替换成下一小节内容 + 徽标。
     本组钉：替换内容、徽标、预告行不被"已弹"刷白、待命球落在预告行、
...
const badgeOf = els => {
  const r = row0(els);
  const b = r && r.children.find(c => /(^| )preview-badge( |$)/.test(c.className));
...
/* ================= 场景 T75b：预告行的内容 / 徽标 / 已弹豁免 ================= */
section("T75b 预告行 · ★ 第 1 行换成下一小节 + 徽标「下一小节 · 型名」+ 不被已弹刷白");
{
  ...
  eq(badgeOf(els), "下一小节 · 八音型", "★ 徽标文字 =「下一小节 · 型名」");
...
section("T75d 预告行边界 · ★ 范围末尾不循环：没有「下一小节」，第 1 行不替换、无徽标");
{
...
section("T75e 预告行边界 · ★ 只循环 A 段时，页末预告的是范围起点（四音型），不是歌曲 k+1（八音型）");
{
  ...
  eq(badgeOf(els), "下一小节 · 四音型",
     "★ 徽标也是范围起点的型名（内容与节目单同源：走 arrNextBar，不走 k+1）");
  beat.Controls.stop();
}
