...
```bash
node tools/check-all.js          # 共 13 步：语法 → 架构约束 → 装配完整性 → 零依赖 lint → 版本一致性 → 文档一致性 → ESLint（可选）→ 类型检查（可选）→ DOM 引用 → 浏览器冒烟（环境可选）→ 全量测试 → 死循环看门狗 → 行覆盖率
...
**核心闸门零依赖、零安装，Node ≥ 22 直接跑**——不装任何东西也能跑完整套检查。13 步里有 3 步可能标 ⊘，并刻意区分"没查"与"查了通过"（否则汇总就在撒谎）。**另外还有第三类状态 ⚠「工具故障」**（v2.8.6 新增）：子进程压根没起来 ≠ 被检项没通过，汇总会把它单列出来，因为两者的处置动作完全不同——一个去查环境，一个去查代码：
...
当前 `v2.8.6`。逐版本变更历史见 [CHANGELOG.md](CHANGELOG.md)，完整操作说明见应用内「使用方法」页。按功能域归纳：
