...
当前 `v2.9.0`。逐版本变更历史见 [CHANGELOG.md](CHANGELOG.md)，完整操作说明见应用内「使用方法」页。按功能域归纳：
...
- **三区预设库与双声部**（v2.5；三区常显于 v2.9.0）：预设库按**内容**分「节拍 / 扫弦 / 自定义」三区**常显**堆叠——带扫弦记谱（`dir` 或 `zone`）的型进「扫弦」区，其余内置型进「节拍」区，自定义型另成第三区；各型**速度、拍号、音量、Swing、音色完全共享**。带扫弦记谱的谱上**每一拍都出节拍音**，扫弦声叠在记谱位置（同时刻，不推后）；音量分「节拍」（拍点 / 预备拍 / 歌词提示音）与「扫弦」两条独立滑条
