...
当前 `v2.10.7`。逐版本变更历史见 [CHANGELOG.md](CHANGELOG.md)，完整操作说明见应用内「使用方法」页。按功能域归纳：
...
- **可视化**：弹跳球预判落点、十六分小格、当前高亮、同屏行数档位（1–4 行可调，**两种模式都是「N 行窗口」**：曲式窗口与歌词轨共用、型短于 N 绕回重复铺满、型长于 N 按 N 行翻页）、双主题（经典 / 观测台）
- **练习工具**：变速训练（自动爬坡）、练习量控制（N 小节 / N 分钟自动停）、7 天爬升计划、听辨训练（节奏默写）、练习统计（时长 / 连续天数 / 速度纪录 / 趋势，可导出 JSON）
- **编排与多轨**：曲式编排（多段串成歌曲、范围循环、跳段）、整首连播、**播放范围双滑块**（侧栏拖动自由选"从第几段练到第几段"，两个滑块拖到重合 = 只练这一段）、三区预设库（节拍 / 扫弦 / 自定义）、双声部（节拍 + 扫弦各一条音量条）
...
- **曲式编排**（v2.0）：一首歌 = 若干段、每段 = 一个节奏型 × N 遍；「起/终」设范围 + 范围循环 = 只练副歌，播放中可跳段。**整首须同拍号**。★ v2.10.4：侧栏「自定义」区里那条**播放范围双滑块**是同一个「起/终」的顺手入口——拖着选"第 3 段到第 7 段"（旧版那排段号胶囊只能点出"只循环某一段"）；两个滑块**拖到重合**就是那档语义；滑块只表达**范围**，"现在播到第几段"看它上方「整首连播」按钮旁的状态说明
